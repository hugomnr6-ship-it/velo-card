 ...
 * Returns L'Échappée de la Semaine for a given week (defaults to current week).
 */
export async function GET(request: Request) {
  ...
  try {
    // Fetch Échappée entries with user info
    const { data: echappeeEntries, error } = await supabaseAdmin
    ...
    return handleApiError(err, "ECHAPPEE_GET");
  }
}
